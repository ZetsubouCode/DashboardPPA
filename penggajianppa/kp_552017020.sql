...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 09, 2020 at 08:28 AM
...
('budi', '00dfc53ee86af02e742515cdcf075ed3', 'user', 'default.jpg', '1504149');
...
--
-- Table structure for table `view_gaji_mentor`
--

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_gaji_mentor` AS select `pg`.`no_penggajian` AS `No`,`pg`.`tanggal_penggajian` AS `Tanggal`,`pg`.`bulan` AS `Bulan`,`pg`.`tahun` AS `Tahun`,`pg`.`nip` AS `NIP`,`p`.`nama_pegawai` AS `Nama Pegawai`,`j`.`nama_jabatan` AS `jabatan`,(`tm`.`gaji` * `pk`.`jumlah_anak`) AS `Gaji Pokok`,`tm`.`bonus` AS `<80%`,(`tm`.`kunjungan` * `pk`.`j_kunjungan`) AS `Kunjungan`,(`tm`.`tutorial` * `pk`.`j_pertemuan`) AS `Tutorial`,(`tm`.`lesson` * `pk`.`lesson`) AS `Lesson Plan`,`pg`.`Evaluasi` AS `Evaluasi Kinerja`,`pg`.`inkeh` AS `Intensif Kehadiran`,`pg`.`insfile` AS `Intensif File Data` from ((((`t_penggajian` `pg` join `t_pk` `pk` on((`pk`.`id` = `pg`.`id_pk`))) join `t_pegawai` `p` on((`p`.`nip` = `pg`.`nip`))) join `tb_jabatan` `j` on((`j`.`kode` = `p`.`id_jabatan`))) join `t_tarif_mentor` `tm` on((`pk`.`id_gaji` = `tm`.`id_gaji`))) where (`j`.`kategori_jabatan` like 'Mentor');
-- Error reading data: (#1356 - View 'kp_552017020.view_gaji_mentor' references invalid table(s) or column(s) or function(s) or definer/invoker of view lack rights to use them)
